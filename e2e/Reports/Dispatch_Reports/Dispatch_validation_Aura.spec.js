...
test.describe('Dispatch Report Data Field Validation', () => {
    ...
    test('Verify Company Name source accuracy', async ({ page }) => {
        ...
        // Verify Company Names are accurate and consistent (should be Automation company)
        let invalidCompanyNames = 0;
        const companyNames = new Set();
        const validCompanies = ['automation company'];
        ...
        parsedData.rows.forEach((row, index) => {
            ...
            if (!companyName || companyName.toString().trim() === '') {
                ...
                if (!validCompanies.some(validCompany => companyLower.includes(validCompany))) {
                    console.log(`[DispatchValidation] Row ${index + 1}: Company "${company}" (may be valid - not strictly Automation company)`);
                }
            }
        });
    ...
    test('Verify Proof Status accuracy', async ({ page }) => {
        console.log('[DispatchValidation] Testing Proof Status validation for WVRD_9th Ave and JG Strydom Rd_62...');
        ...
        const targetGroupName = 'WVRD_9th Ave and JG Strydom Rd_62';
    ...
    test('Verify Extended Responder Status values', async ({ page }) => {
        console.log('[DispatchValidation] Testing Extended Responder Status validation for WVRD_9th Ave and JG Strydom Rd_62...');
    ...
    test('Verify Both Column Status Pairs', async ({ page }) => {
        console.log('[DispatchValidation] Testing Both Column Status Pairs validation for WVRD_9th Ave and JG Strydom Rd_62...');
...
test.describe('Dispatch Report CSV Format Validation', () => {
    ...
    test('Verify WVRD_9th Ave Group Name Data Presence', async ({ page }) => {
        console.log('[DispatchValidation] Testing WVRD_9th Ave and JG Strydom Rd_62 data presence...');
        
        const reportName = `WVRD_9th Ave Test ${TEST_CONFIG.testData.currentTime}`;
        ...
        // Parse the downloaded file
        const parsedData = await saveAndParseDownload(download, 'WVRD_9th_Ave_Validation');
        ...
        if (parsedData.rows.length === 0) {
            console.log('[DispatchValidation] ℹ️ Report contains no data rows - cannot validate WVRD_9th Ave and JG Strydom Rd_62 presence');
            console.log('[DispatchValidation] ⚠️ Consider using a broader date range to capture WVRD_9th Ave and JG Strydom Rd_62 data');
            return;
        }
        
        // Search for WVRD_9th Ave and JG Strydom Rd_62 entries in Group Name
        let wvrdCount = 0;
        const wvrdEntries = [];
        
        parsedData.rows.forEach((row, index) => {
            const groupName = row[groupNameColumnIndex];
            if (groupName && groupName.toString().includes('WVRD_9th Ave and JG Strydom Rd_62')) {
                wvrdCount++;
                wvrdEntries.push({
                    row: index + 1,
        ...
        console.log(`[DispatchValidation] 📊 Found ${wvrdCount} entries for WVRD_9th Ave and JG Strydom Rd_62`);
        
        if (wvrdCount > 0) {
            console.log('[DispatchValidation] ✅ WVRD_9th Ave and JG Strydom Rd_62 data found in dispatch report:');
            wvrdEntries.forEach((entry, index) => {
            ...
            // Verify that all WVRD_9th Ave entries have proper data
            const validEntries = wvrdEntries.filter(entry => 
                entry.incidentId !== 'N/A' && entry.incidentId !== '' && 
                entry.siteName !== 'N/A' && entry.siteName !== ''
            );
            
            console.log(`[DispatchValidation] ✅ Valid WVRD_9th Ave entries: ${validEntries.length}/${wvrdCount}`);
            
            // Expect at least some valid entries if data exists
            expect(validEntries.length).toBeGreaterThan(0);
        } else {
            console.log('[DispatchValidation] ℹ️ No WVRD_9th Ave and JG Strydom Rd_62 entries found in current date range');
        ...
        console.log('[DispatchValidation] ✅ Test completed: Verify WVRD_9th Ave Group Name Data Presence');
    });

    test('Verify Dispatch flow for WVRD_9th Ave', async ({ page }) => {
        console.log('[DispatchValidation] Testing WVRD_9th Ave and JG Strydom Rd_62 dispatch workflow statuses...');
        
        const reportName = `WVRD_9th Ave Dispatch Flow Test ${TEST_CONFIG.testData.currentTime}`;
        ...
        // Parse the downloaded file
        const parsedData = await saveAndParseDownload(download, 'WVRD_9th_Ave_Dispatch_Flow_Validation');
        ...
        if (parsedData.rows.length === 0) {
            console.log('[DispatchValidation] ℹ️ Report contains no data rows - cannot validate WVRD_9th Ave dispatch flow');
            console.log('[DispatchValidation] ⚠️ Consider using a broader date range to capture WVRD_9th Ave dispatch workflow data');
            return;
        }
        
        // Filter rows where Group Name = "WVRD_9th Ave and JG Strydom Rd_62"
        const wvrdRows = [];
        parsedData.rows.forEach((row, index) => {
            const groupName = row[groupNameColumnIndex];
            if (groupName && groupName.toString().includes('WVRD_9th Ave and JG Strydom Rd_62')) {
        ...
        console.log(`[DispatchValidation] 📊 Found ${wvrdRows.length} WVRD_9th Ave entries for dispatch flow analysis`);
        
        if (wvrdRows.length === 0) {
            console.log('[DispatchValidation] ℹ️ No WVRD_9th Ave and JG Strydom Rd_62 entries found in current date range');
            console.log('[DispatchValidation] 📋 Available Group Names sample (first 5):');
        ...
        // Extract all Proof Status values for WVRD_9th Ave entries
        const proofStatuses = wvrdRows
            .map(row => row.proofStatus)
            .filter(status => status && status !== '');
        
        const uniqueStatuses = [...new Set(proofStatuses)];
        
        console.log('[DispatchValidation] 📋 Found Proof Status values for WVRD_9th Ave:');
        ...
        console.log('[DispatchValidation] 📊 Dispatch Workflow Analysis for WVRD_9th Ave:');
        console.log(`[DispatchValidation] Total entries: ${wvrdRows.length}`);
        ...
        console.log('[DispatchValidation] ✅ All expected dispatch workflow statuses found for WVRD_9th Ave');
        
        // Additional validation: Ensure we have actual data to validate
        expect(wvrdRows.length).toBeGreaterThan(0);
        expect(proofStatuses.length).toBeGreaterThan(0);
        
        console.log('[DispatchValidation] ✅ Test completed: Verify Dispatch flow for WVRD_9th Ave');
    });
    ...
    test('Cleanup any remaining test reports', async ({ page }) => {
        ...
        try {
            ...
            const testReportPatterns = [
                ...
                'CSV Validation Test',
                'WVRD_9th Ave Test',
                'WVRD_9th Ave Dispatch Flow Test'
            ];
